...
| R7a | Completed **cash** rows with `shift_id NULL` | **WARN** (deviation from "High" — deliberate: `STRICT_SHIFT_CASH` is off and the system explicitly permits cash without an open shift, with a cashier toast) | Owner/manager can assign same-day rows into a shift via "ผูกเงินสดเข้ารอบย้อนหลัง" on /pos/shifts (Phase 16G-A — reason required, audited as `cash_shift_backfill`). Rows from days with no shift at all stay WARN. If the count grows during service, retrain on shift-open or enable `STRICT_SHIFT_CASH` |
